...
try:
    ...
    byte_array = bytearray(photo)

    def createDict(cid, th_name, eng_name, dob, gender, address, card_issuer, issue_date, expire_date):
        card_dict                   = dict()
        card_dict['cid']            = cid
        card_dict['th_name']        = replaceAsterick(th_name)
        card_dict['eng_name']       = replaceAsterick(eng_name)
        card_dict['dob']            = dob
        card_dict['gender']         = gender
        card_dict['address']        = replaceAsterick(address)
        card_dict['card_issuer']    = card_issuer
        card_dict['issue_date']     = issue_date
        card_dict['expire_date']    = expire_date
        # print(card_dict)
        return card_dict

    def replaceAsterick(word):
        for letter in word:
            if letter == '#':
    ...
    dataToInsert = createDict(cid, th_name, eng_name, dob, gender, address, card_issuer, issue_date, expire_date)

    y = json.dumps(dataToInsert)
    # print(y)

    with open("D:/allindex/idcard/idcard.txt", "w") as outfile:
        outfile.write(y)
    outfile.close()
